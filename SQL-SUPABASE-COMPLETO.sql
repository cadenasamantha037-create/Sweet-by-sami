...
-- ============================================================
-- SWEET BY SAMI · V37
-- Dedicatoria + reembolsos + soporte admin
-- ============================================================

begin;

alter table public.orders add column if not exists dedication_from text;
alter table public.orders add column if not exists dedication_to text;
alter table public.orders add column if not exists refund_amount numeric(10,2) not null default 0;
...
begin
  ...
  insert into public.orders(
    ...
    nullif(trim(coalesce(p_payload->>'address','')),''),nullif(trim(coalesce(p_payload->>'reference','')),''),
    v_shipping_recipient_name,v_shipping_recipient_phone,v_shipping_recipient_ci,
    nullif(trim(coalesce(p_payload->>'dedication_from','')),''),
    nullif(trim(coalesce(p_payload->>'dedication_to','')),''),
    v_preparation_mode,v_payment_method,v_receipt_path,
    case when v_payment_method='cash' then 'cash_pending' else 'pending_review' end
  ) returning id,order_serial into v_order_id,v_order_serial;
